...
class PlatformSelect(Select):
    def __init__(self, ctx):
        self.ctx = ctx
        options = [
            discord.SelectOption(label="Youtube", description="Search with Youtube (preferred)", value="youtube"),
            discord.SelectOption(label="Spotify", description="Search with Spotify", value="spotify"),
            discord.SelectOption(label="Cancel", description="Cancel the request", value="cancel")
        ]
        super().__init__(placeholder="Select a platform...", options=options)

    async def callback(self, interaction):
       if self.values[0] == "cancel":
          await interaction.response.send_message("Cancelling request...")
...
class PlatformView(View):
    def __init__(self, ctx):
        ...
        self.add_item(PlatformSelect(ctx))

class SearchSelect(Select):
    def __init__(self, ctx, tracks: list[wavelink.Playable], player: wavelink.Player):
        ...
        if len(tracks) <= 0:
            options = [discord.SelectOption(label="Failed to find tracks", description="Failed to find any suitable tracks which fit your query", value="cancel")]
        else:
            options = [discord.SelectOption(label=track.title, description=track.uri, value=i) for i, track in enumerate(self.tracks)]
            options.append(discord.SelectOption(label="Cancel", description="Cancel the interaction", value="cancel"))

        super().__init__(placeholder="Select a song...", options=options)

    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "cancel":
            return await interaction.response.send_message("Cancelling request...")
...
class SearchView(View):
    def __init__(self, ctx, tracks, player):
       super().__init__(timeout=30)
       self.add_item(SearchSelect(ctx, tracks, player))
...
class PlayingView(View):
    ...
    async def _repeat(self, interaction: discord.Interaction, button: discord.ui.Button):
      if interaction.user.id != self.user_id:
        return await interaction.response.send_message("Only command caller can do that.", ephemeral=True)
      if self.player:
          if self.player.queue.mode == wavelink.enums.QueueMode.loop_all:
              self.player.queue.mode = wavelink.enums.QueueMode.normal
              await interaction.response.send_message("```🔁 Repeat off.```", ephemeral=True)
          else:
              self.player.queue.mode = wavelink.enums.QueueMode.loop_all
